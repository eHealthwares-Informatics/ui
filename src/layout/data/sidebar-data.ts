import {
  AudioWaveform,
  Boxes,
  Brain,
  ChartColumn,
  CircleDashed,
  MessageSquare,
  Cog,
  Command,
  ContactRound,
  Cpu,
  CreditCard,
  FileText,
  Server,
  GitBranch,
  HandCoins,
  LayoutDashboard,
  Layers3,
  Microscope,
  NotebookPen,
  Package,
  PackageSearch,
  Pill,
  Radio,
  ReceiptText,
  Scale,
  Shield,
  ShoppingCart,
  Shuffle,
  Tags,
  Truck,
  Users,
  MapPin,
  KeyRound,
  Waypoints,
  Workflow,
  Store,
  BookOpen,
  HeartPulse,
  Receipt,
  ShieldCheck,
  MessagesSquare,
  Braces,
  Building2,
  ClipboardCheck,
  Stethoscope,
  CalendarDays,
  UserRound,
  UserCog,
  Bed,
  FileSpreadsheet,
  FilePlus2,
  Share2,
  ClipboardList,
  Globe,
  Wallet,
  Smartphone,
  Settings,
} from 'lucide-react';
import type { ModuleId } from '@/features/shared/module-data';
import { NavItem, type SidebarData } from '../types';

export function filterNavGroupsByModule(navGroups: NavItem[], selectedModule: ModuleId): NavItem[] {
  return navGroups
    .map((group: NavItem) => ({
      ...group,
      items: (group.items || []).filter(
        (item) =>
          selectedModule === 'admin' || !item.modules || item.modules.includes(selectedModule)
      ),
      url: undefined,
    }))
    .filter((group) => group.items.length > 0);
}

export const sidebarData: SidebarData = {
  user: {
    name: 'RxSoft User',
    email: 'Admin console',
    avatar: '',
  },
  teams: [
    {
      name: 'RxSoft',
      logo: Command,
      plan: 'Pharmacy Admin',
      moduleId: 'rxsoft',
    },
    {
      name: 'Switch',
      logo: AudioWaveform,
      plan: 'Messaging & Routing',
      moduleId: 'communication',
    },
    {
      name: 'Conversation',
      logo: MessageSquare,
      plan: 'Workflow Chat',
      moduleId: 'conversation',
    },
    {
      name: 'Coding Concept',
      logo: Boxes,
      plan: 'Terminology',
      moduleId: 'coding-concept',
    },
    {
      name: 'LIS',
      logo: Microscope,
      plan: 'Laboratory',
      moduleId: 'lis',
    },
    {
      name: 'EMR',
      logo: HeartPulse,
      plan: 'Electronic Medical Record',
      moduleId: 'emr',
    },
    {
      name: 'Identity',
      logo: ShieldCheck,
      plan: 'Identity & Access',
      moduleId: 'identity',
    },
  ],
  navGroups: [
    {
      title: 'Overview',
      icon: LayoutDashboard,
      items: [
        {
          title: 'Dashboard',
          url: '/dashboard/sales',
          icon: LayoutDashboard,
          modules: ['rxsoft'],
        },
        {
          title: 'Purchases',
          url: '/dashboard/purchases',
          icon: ShoppingCart,
          modules: ['rxsoft'],
        },
        {
          title: 'Reports',
          url: '/rxsoft/reports',
          icon: ChartColumn,
          modules: ['rxsoft'],
        },
        {
          title: 'Audit Logs',
          url: '/rxsoft/audit-logs',
          icon: Package,
          modules: ['admin'],
        },
      ],
    },
    {
      title: 'Shop',
      icon: Globe,
      items: [
        {
          title: 'Website',
          url: '/shop',
          icon: Globe,
          modules: ['rxsoft'],
        },
        {
          title: 'Purchases',
          url: '/shop/purchases',
          icon: ShoppingCart,
          modules: ['rxsoft'],
        },
        {
          title: 'POS',
          url: '/shop/pos',
          icon: Store,
          modules: ['rxsoft'],
        },
      ],
    },
    {
      title: 'Catalog',
      icon: BookOpen,
      items: [
        {
          title: 'Products',
          url: '/rxsoft/items',
          icon: Package,
          modules: ['rxsoft'],
        },
        {
          title: 'Categories',
          url: '/rxsoft/categories',
          icon: Tags,
          modules: ['rxsoft'],
        },
        {
          title: 'UOMs',
          url: '/rxsoft/uoms',
          icon: Scale,
          modules: ['rxsoft'],
        },
        {
          title: 'Manufacturers',
          url: '/rxsoft/manufacturers',
          icon: Truck,
          modules: ['rxsoft'],
        },
        {
          title: 'Price Lists',
          url: '/rxsoft/price-lists',
          icon: Tags,
          modules: ['rxsoft'],
        },
        {
          title: 'Price List Items',
          url: '/rxsoft/price-list-items',
          icon: ReceiptText,
          modules: ['rxsoft'],
        },
      ],
    },
    {
      title: 'People',
      icon: Users,
      items: [
        {
          title: 'Users',
          url: '/rxsoft/users',
          icon: Users,
          modules: ['rxsoft', 'admin'],
        },
        {
          title: 'Customers',
          url: '/rxsoft/customers',
          icon: ContactRound,
          modules: ['rxsoft'],
        },
        {
          title: 'Suppliers',
          url: '/rxsoft/suppliers',
          icon: Truck,
          modules: ['rxsoft'],
        },
        {
          title: 'Roles',
          url: '/rxsoft/roles',
          icon: Shield,
          modules: ['rxsoft', 'admin'],
        },
        {
          title: 'Roles Request',
          url: '/rxsoft/role-requests',
          icon: ShieldCheck,
          modules: ['rxsoft', 'admin'],
        },
      ],
    },
    {
      title: 'Operations',
      icon: Workflow,
      items: [
        {
          title: 'Sales',
          url: '/rxsoft/sales',
          icon: ShoppingCart,
          modules: ['rxsoft'],
        },
        {
          title: 'Sales Lines',
          url: '/rxsoft/sales-lines',
          icon: ShoppingCart,
          modules: ['rxsoft'],
        },
        {
          title: 'Receivables',
          url: '/rxsoft/receivables',
          icon: HandCoins,
          modules: ['rxsoft'],
        },
        {
          title: 'Purchases',
          url: '/rxsoft/purchases',
          icon: ReceiptText,
          modules: ['rxsoft'],
        },
        {
          title: 'Inventory',
          url: '/rxsoft/inventory',
          icon: Boxes,
          modules: ['rxsoft'],
        },
        {
          title: 'Goods Receiving',
          url: '/rxsoft/receiving',
          icon: PackageSearch,
          modules: ['rxsoft'],
        },
        {
          title: 'Website Orders',
          url: '/rxsoft/website-orders',
          icon: ShoppingCart,
          modules: ['rxsoft'],
        },
        {
          title: 'Branches',
          url: '/rxsoft/branches',
          icon: GitBranch,
          modules: ['rxsoft'],
        },
        {
          title: 'Settings',
          url: '/rxsoft/settings',
          icon: Cog,
          modules: ['rxsoft', 'admin'],
        },
      ],
    },
    {
      title: 'Payments',
      icon: CreditCard,
      items: [
        {
          title: 'Payments',
          url: '/rxsoft/payments',
          icon: Wallet,
          modules: ['rxsoft'],
        },
        {
          title: 'Payment Transactions',
          url: '/rxsoft/payment-transactions',
          icon: CreditCard,
          modules: ['rxsoft'],
        },
        {
          title: 'Payment Methods',
          url: '/rxsoft/payment-methods',
          icon: Wallet,
          modules: ['rxsoft'],
        },
        {
          title: 'Payment Providers',
          url: '/rxsoft/payment-providers',
          icon: Wallet,
          modules: ['rxsoft'],
        },
        {
          title: 'Organisation Payment Providers',
          url: '/rxsoft/organisation-payment-providers',
          icon: Wallet,
          modules: ['rxsoft'],
        },
        {
          title: 'POS Terminals',
          url: '/rxsoft/pos-terminals',
          icon: Smartphone,
          modules: ['rxsoft'],
        },
        {
          title: 'Insurance Providers',
          url: '/rxsoft/insurance-providers',
          icon: ShieldCheck,
          modules: ['rxsoft'],
        },
      ],
    },
    {
      title: 'Finance',
      icon: Receipt,
      items: [
        {
          title: 'Chart of Accounts',
          url: '/rxsoft/gl-accounts',
          icon: BookOpen,
          modules: ['rxsoft'],
        },
        {
          title: 'Journals',
          url: '/rxsoft/journals',
          icon: NotebookPen,
          modules: ['rxsoft'],
        },
        {
          title: 'Journal Entries',
          url: '/rxsoft/journal-entries',
          icon: NotebookPen,
          modules: ['rxsoft'],
        },
        {
          title: 'Journal Entry Lines',
          url: '/rxsoft/journal-entry-lines',
          icon: NotebookPen,
          modules: ['rxsoft'],
        },
        {
          title: 'Trial Balance',
          url: '/rxsoft/reports/trial-balance',
          icon: Scale,
          modules: ['rxsoft'],
        },
        {
          title: 'Balance Sheet',
          url: '/rxsoft/reports/balance-sheet',
          icon: FileText,
          modules: ['rxsoft'],
        },
        {
          title: 'Income Statement',
          url: '/rxsoft/reports/income-statement',
          icon: ChartColumn,
          modules: ['rxsoft'],
        },
      ],
    },
    {
      title: 'Website',
      icon: Store,
      items: [
        {
          title: 'Health Concerns',
          url: '/rxsoft/website-health-concerns',
          icon: HeartPulse,
          modules: ['rxsoft'],
        },
        {
          title: 'Blog Articles',
          url: '/rxsoft/website-articles',
          icon: BookOpen,
          modules: ['rxsoft'],
        },
        {
          title: 'Prescriptions',
          url: '/rxsoft/website-prescriptions',
          icon: FileText,
          modules: ['rxsoft'],
        },
      ],
    },
    {
      title: 'eHealthwares',
      icon: Globe,
      items: [
        {
          title: 'Hero Slides',
          url: '/rxsoft/ehealthwares-hero-slides',
          icon: Store,
          modules: ['website'],
        },
        {
          title: 'Products',
          url: '/rxsoft/ehealthwares-products',
          icon: Package,
          modules: ['website'],
        },
        {
          title: 'Services',
          url: '/rxsoft/ehealthwares-services',
          icon: PackageSearch,
          modules: ['website'],
        },
        {
          title: 'Categories',
          url: '/rxsoft/ehealthwares-categories',
          icon: Tags,
          modules: ['website'],
        },
        {
          title: 'Articles',
          url: '/rxsoft/ehealthwares-articles',
          icon: BookOpen,
          modules: ['website'],
        },
        {
          title: 'Testimonials',
          url: '/rxsoft/ehealthwares-testimonials',
          icon: HeartPulse,
          modules: ['website'],
        },
        {
          title: 'Partners',
          url: '/rxsoft/ehealthwares-partners',
          icon: HandCoins,
          modules: ['website'],
        },
        {
          title: 'Team',
          url: '/rxsoft/ehealthwares-team',
          icon: Users,
          modules: ['website'],
        },
        {
          title: 'Investor Data',
          url: '/rxsoft/ehealthwares-investors',
          icon: ChartColumn,
          modules: ['website'],
        },
        {
          title: 'Careers',
          url: '/rxsoft/ehealthwares-careers',
          icon: FileText,
          modules: ['website'],
        },
        {
          title: 'Site Sections',
          url: '/rxsoft/ehealthwares-sections',
          icon: Layers3,
          modules: ['website'],
        },
        {
          title: 'Site Settings',
          url: '/rxsoft/ehealthwares-settings',
          icon: Cog,
          modules: ['website'],
        },
        {
          title: 'Contact Submissions',
          url: '/rxsoft/ehealthwares-contact-submissions',
          icon: ContactRound,
          modules: ['website'],
        },
      ],
    },
    {
      title: 'Administration',
      icon: ShieldCheck,
      items: [
        {
          title: 'Organizations',
          url: '/rxsoft/organizations',
          icon: Shield,
          modules: ['admin'],
        },
        {
          title: 'Locations',
          url: '/rxsoft/stock-locations',
          icon: Store,
          modules: ['admin'],
        },
        {
          title: 'Warehouses',
          url: '/rxsoft/warehouses',
          icon: Building2,
          modules: ['admin'],
        },
      ],
    },
    {
      title: 'Configuration',
      icon: Settings,
      items: [
        {
          title: 'User Config',
          url: '/rxsoft/user-config',
          icon: UserCog,
          modules: ['rxsoft'],
        },
        {
          title: 'Organisation Config',
          url: '/rxsoft/organisation-config',
          icon: Building2,
          modules: ['rxsoft'],
        },
      ],
    },
    {
      title: 'Identity & Access',
      icon: ShieldCheck,
      items: [
        {
          title: 'Users',
          url: '/identity/users',
          icon: UserCog,
          modules: ['identity'],
        },
        {
          title: 'Roles',
          url: '/identity/roles',
          icon: Shield,
          modules: ['identity'],
        },
        {
          title: 'Permissions',
          url: '/identity/permissions',
          icon: KeyRound,
          modules: ['identity'],
        },
        {
          title: 'Organizations',
          url: '/identity/organizations',
          icon: Building2,
          modules: ['identity'],
        },
        {
          title: 'Locations',
          url: '/identity/locations',
          icon: MapPin,
          modules: ['identity'],
        },
      ],
    },
    {
      title: 'Question',
      icon: CircleDashed,
      items: [
        {
          title: 'Questionnaires',
          url: '/conversation/questionnaires',
          icon: FileText,
          modules: ['conversation'],
        },
        {
          title: 'Questions',
          url: '/conversation/questions',
          icon: CircleDashed,
          modules: ['conversation'],
        },
        {
          title: 'Option Lists',
          url: '/conversation/option-lists',
          icon: ClipboardList,
          modules: ['conversation'],
        },
      ],
    },
    {
      title: 'Conversation',
      icon: MessagesSquare,
      items: [
        {
          title: 'Conversations',
          url: '/conversation',
          icon: MessageSquare,
          modules: ['conversation'],
        },
        {
          title: 'Participants',
          url: '/conversation/participants',
          icon: ContactRound,
          modules: ['conversation'],
        },
        {
          title: 'Exchanges',
          url: '/conversation/exchanges',
          icon: FileText,
          modules: ['conversation'],
        },
        {
          title: 'Projections',
          url: '/conversation/projections',
          icon: ContactRound,
          modules: ['conversation'],
        },
        {
          title: 'Invites',
          url: '/conversation/invites',
          icon: FileText,
          modules: ['conversation'],
        },
      ],
    },
    {
      title: 'Chat',
      icon: MessageSquare,
      items: [
        {
          title: 'Chats',
          url: '/conversation/chats',
          icon: MessageSquare,
          modules: ['conversation'],
        },
      ],
    },
    {
      title: 'Channels',
      icon: Radio,
      items: [
        {
          title: 'Channels',
          url: '/conversation/channels',
          icon: Radio,
          modules: ['conversation'],
        },
      ],
    },
    {
      title: 'AI',
      icon: Brain,
      items: [
        {
          title: 'Providers',
          url: '/conversation/ai/providers',
          icon: Server,
          modules: ['conversation'],
        },
        {
          title: 'Models',
          url: '/conversation/ai/models',
          icon: Cpu,
          modules: ['conversation'],
        },
        {
          title: 'Processors',
          url: '/conversation/ai/processors',
          icon: Boxes,
          modules: ['conversation'],
        },
        {
          title: 'Instructions',
          url: '/conversation/ai/instructions',
          icon: Brain,
          modules: ['conversation'],
        },
        {
          title: 'Eval Logs',
          url: '/conversation/ai/eval-logs',
          icon: FileText,
          modules: ['conversation'],
        },
        {
          title: 'Costs',
          url: '/conversation/ai/costs',
          icon: ChartColumn,
          modules: ['conversation'],
        },
        {
          title: 'Request Logs',
          url: '/conversation/ai/request-logs',
          icon: Receipt,
          modules: ['conversation'],
        },
        {
          title: 'AI Config',
          url: '/conversation/ai/config',
          icon: Settings,
          modules: ['conversation'],
        },
      ],
    },
    {
      title: 'Workflow',
      icon: Workflow,
      items: [
        {
          title: 'Workflows',
          url: '/conversation/workflows',
          icon: Workflow,
          modules: ['conversation'],
        },
        {
          title: 'Workflow Configuration',
          url: '/conversation/workflow-configuration',
          icon: Workflow,
          modules: ['conversation'],
        },
        {
          title: 'Workflow Instances',
          url: '/conversation/workflow-instances',
          icon: Shuffle,
          modules: ['conversation'],
        },
        {
          title: 'Workflow Events',
          url: '/conversation/workflow-events',
          icon: Waypoints,
          modules: ['conversation'],
        },
      ],
    },
    {
      title: 'Switch',
      icon: Waypoints,
      items: [
        {
          title: 'Application Entities',
          url: '/communication/aes',
          icon: MessageSquare,
          modules: ['communication'],
        },
        {
          title: 'Routes',
          url: '/communication/routing',
          icon: MessageSquare,
          modules: ['communication'],
        },
        {
          title: 'Mapping',
          url: '/communication/mapping',
          icon: MessageSquare,
          modules: ['communication'],
        },
        {
          title: 'Event Traces',
          url: '/communication/audit-center',
          icon: FileText,
          modules: ['communication'],
        },

        {
          title: 'Flow Graph',
          url: '/communication/flow-graph',
          icon: Layers3,
          modules: ['communication'],
        },
        {
          title: 'Trace Explorer',
          url: '/communication/trace-explorer',
          icon: Waypoints,
          modules: ['communication'],
        },
        {
          title: 'Message Tester',
          url: '/communication/message-tester',
          icon: MessageSquare,
          modules: ['communication'],
        },
      ],
    },
    {
      title: 'Coding Concept',
      icon: Braces,
      items: [
        {
          title: 'Concept Registry',
          url: '/coding-concept',
          icon: Layers3,
          modules: ['coding-concept'],
        },
        {
          title: 'Search',
          url: '/coding-concept/search',
          icon: Waypoints,
          modules: ['coding-concept'],
        },
        {
          title: 'Match',
          url: '/coding-concept/match',
          icon: GitBranch,
          modules: ['coding-concept'],
        },
        {
          title: 'Upload',
          url: '/coding-concept/upload',
          icon: FileText,
          modules: ['coding-concept'],
        },
      ],
    },
    {
      title: 'Facility',
      icon: Building2,
      items: [
        {
          title: 'Facilities',
          url: '/coding-concept/facilities',
          icon: Building2,
          modules: ['coding-concept'],
        },
        {
          title: 'Pharmacies',
          url: '/coding-concept/facilities/pharmacies',
          icon: Building2,
          modules: ['coding-concept'],
        },
        {
          title: 'Hospitals',
          url: '/coding-concept/facilities/hospitals',
          icon: Building2,
          modules: ['coding-concept'],
        },
        {
          title: 'Laboratories',
          url: '/coding-concept/facilities/laboratories',
          icon: Building2,
          modules: ['coding-concept'],
        },
        {
          title: 'States',
          url: '/coding-concept/facilities/states',
          icon: Building2,
          modules: ['coding-concept'],
        },
        {
          title: 'LGAs',
          url: '/coding-concept/facilities/lgas',
          icon: Building2,
          modules: ['coding-concept'],
        },
        {
          title: 'Wards',
          url: '/coding-concept/facilities/wards',
          icon: Building2,
          modules: ['coding-concept'],
        },
        {
          title: 'Facility Types',
          url: '/coding-concept/facilities/types',
          icon: Building2,
          modules: ['coding-concept'],
        },
        {
          title: 'Facility Levels',
          url: '/coding-concept/facilities/levels',
          icon: Building2,
          modules: ['coding-concept'],
        },
      ],
    },
    {
      title: 'Drugs',
      icon: Pill,
      items: [
        {
          title: 'Generic Drugs',
          url: '/coding-concept/generic-drugs',
          icon: Pill,
          modules: ['coding-concept'],
        },
        {
          title: 'Generic Products',
          url: '/coding-concept/generic-products',
          icon: Pill,
          modules: ['coding-concept'],
        },
        {
          title: 'Pharmaceutics',
          url: '/coding-concept/pharmaceutics',
          icon: Pill,
          modules: ['coding-concept'],
        },
        {
          title: 'Drug Components',
          url: '/coding-concept/drug-components',
          icon: Layers3,
          modules: ['coding-concept'],
        },
        {
          title: 'Dosage Forms',
          url: '/coding-concept/dosage-forms',
          icon: Pill,
          modules: ['coding-concept'],
        },
        {
          title: 'Formulations',
          url: '/coding-concept/formulations',
          icon: Pill,
          modules: ['coding-concept'],
        },
        {
          title: 'Manufacturers',
          url: '/coding-concept/manufacturers',
          icon: Pill,
          modules: ['coding-concept'],
        },
        {
          title: 'Drug Classifications',
          url: '/coding-concept/drug-classifications',
          icon: Tags,
          modules: ['coding-concept'],
        },
      ],
    },
    {
      title: 'Tests',
      icon: Scale,
      items: [
        {
          title: 'Test Definitions',
          url: '/lis/test-definitions',
          icon: Scale,
          modules: ['lis'],
        },
        {
          title: 'Reference Ranges',
          url: '/lis/reference-ranges',
          icon: Scale,
          modules: ['lis'],
        },
        {
          title: 'Test Categories',
          url: '/lis/test-categories',
          icon: Scale,
          modules: ['lis'],
        },
        {
          title: 'Test Sections',
          url: '/lis/test-sections',
          icon: Scale,
          modules: ['lis'],
        },
        { title: 'Methods', url: '/lis/methods', icon: Scale, modules: ['lis'] },
        { title: 'Panels', url: '/lis/panels', icon: Scale, modules: ['lis'] },
        { title: 'UOMs', url: '/lis/uoms', icon: Scale, modules: ['lis'] },
        { title: 'LOINC', url: '/lis/loinc', icon: Scale, modules: ['lis'] },
      ],
    },
    {
      title: 'Orders',
      icon: FileText,
      items: [
        { title: 'Orders', url: '/lis/orders', icon: FileText, modules: ['lis'] },
        {
          title: 'Orders Dashboard',
          url: '/lis/orders/dashboard',
          icon: LayoutDashboard,
          modules: ['lis'],
        },
        {
          title: 'Workflow',
          url: '/lis/orders/workflow/enter',
          icon: FileText,
          modules: ['lis'],
        },
        { title: 'Patients', url: '/lis/patients', icon: FileText, modules: ['lis'] },
        { title: 'Priorities', url: '/lis/priorities', icon: FileText, modules: ['lis'] },
        { title: 'Programs', url: '/lis/programs', icon: FileText, modules: ['lis'] },
      ],
    },
    {
      title: 'Results',
      icon: NotebookPen,
      items: [
        { title: 'Results', url: '/lis/results', icon: NotebookPen, modules: ['lis'] },
        {
          title: 'Result Signatures',
          url: '/lis/result-signatures',
          icon: NotebookPen,
          modules: ['lis'],
        },
        {
          title: 'Validation Dashboard',
          url: '/lis/validation-dashboard',
          icon: NotebookPen,
          modules: ['lis'],
        },
      ],
    },
    {
      title: 'Samples',
      icon: Boxes,
      items: [
        { title: 'Samples', url: '/lis/samples', icon: Boxes, modules: ['lis'] },
        { title: 'Sample Types', url: '/lis/sample-types', icon: Boxes, modules: ['lis'] },
        { title: 'Locations', url: '/lis/locations', icon: Boxes, modules: ['lis'] },
        {
          title: 'Location Types',
          url: '/lis/location-types',
          icon: Boxes,
          modules: ['lis'],
        },
        {
          title: 'Rejection Reasons',
          url: '/lis/rejection-reasons',
          icon: Boxes,
          modules: ['lis'],
        },
      ],
    },
    {
      title: 'Quality Control',
      icon: ChartColumn,
      items: [
        {
          title: 'QA Checklist',
          url: '/lis/qa-checklist-items',
          icon: ClipboardCheck,
          modules: ['lis'],
        },
        { title: 'QC Lots', url: '/lis/qc-lots', icon: ChartColumn, modules: ['lis'] },
        {
          title: 'QC Results',
          url: '/lis/qc-results',
          icon: ChartColumn,
          modules: ['lis'],
        },
        { title: 'QC Alerts', url: '/lis/qc-alerts', icon: ChartColumn, modules: ['lis'] },
      ],
    },
    {
      title: 'EQA',
      icon: ClipboardCheck,
      items: [
        {
          title: 'EQA Programs',
          url: '/lis/eqa-programs',
          icon: ClipboardCheck,
          modules: ['lis'],
        },
        {
          title: 'EQA Enrollments',
          url: '/lis/eqa-enrollments',
          icon: ClipboardCheck,
          modules: ['lis'],
        },
        {
          title: 'EQA Results',
          url: '/lis/eqa-results',
          icon: ClipboardCheck,
          modules: ['lis'],
        },
      ],
    },
    {
      title: 'Configuration',
      icon: Cog,
      items: [
        { title: 'Statuses', url: '/lis/statuses', icon: Cog, modules: ['lis'] },
        {
          title: 'Attribute Definitions',
          url: '/lis/attribute-definitions',
          icon: Cog,
          modules: ['lis'],
        },
      ],
    },
    {
      title: 'EMR',
      icon: HeartPulse,
      items: [
        { title: 'Dashboard', url: '/emr', icon: LayoutDashboard, modules: ['emr'] },
        {
          title: 'Appointments',
          url: '/emr/appointments',
          icon: CalendarDays,
          modules: ['emr'],
        },
        { title: 'Patients', url: '/emr/patients', icon: UserRound, modules: ['emr'] },
        { title: 'Visits', url: '/emr/visits', icon: Bed, modules: ['emr'] },
        {
          title: 'Encounters',
          url: '/emr/encounters',
          icon: Stethoscope,
          modules: ['emr'],
        },
        {
          title: 'Clinical Requests',
          url: '/emr/requests',
          icon: FilePlus2,
          modules: ['emr'],
        },
        {
          title: 'Referrals',
          url: '/emr/referrals',
          icon: Share2,
          modules: ['emr'],
        },
        {
          title: 'Medications',
          url: '/emr/medications',
          icon: Pill,
          modules: ['emr'],
        },
      ],
    },
    {
      title: 'Wards',
      icon: Bed,
      items: [
        {
          title: 'Wards',
          url: '/emr/wards',
          icon: Bed,
          modules: ['emr'],
        },
        {
          title: 'Beds',
          url: '/emr/wards/beds',
          icon: Bed,
          modules: ['emr'],
        },
        {
          title: 'Admissions',
          url: '/emr/wards/admissions',
          icon: Bed,
          modules: ['emr'],
        },
        {
          title: 'Discharges',
          url: '/emr/wards/discharges',
          icon: Bed,
          modules: ['emr'],
        },
        {
          title: 'Bed Allocations',
          url: '/emr/wards/board',
          icon: Bed,
          modules: ['emr'],
        },
      ],
    },
    {
      title: 'Configuration',
      icon: Cog,
      items: [
        { title: 'Staff', url: '/emr/staff', icon: UserCog, modules: ['emr'] },
        {
          title: 'Departments',
          url: '/emr/departments',
          icon: Building2,
          modules: ['emr'],
        },
        { title: 'Forms', url: '/emr/forms', icon: FileSpreadsheet, modules: ['emr'] },
        { title: 'Tags', url: '/emr/tags', icon: Tags, modules: ['emr'] },
      ],
    },
  ],
};
